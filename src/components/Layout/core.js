...
function getFontSizeBase () {
   if (fontSizeBase) {
      ...
      try {
         ...
         // Dividing by 1 casts possible string to a number
         // which could be a fractional number (so parseInt would not work here)
...
function parseBreakpointCondition (breakpoint) {
   return breakpoint.when.split(' ').reduce((bp, item, i) => {
      switch (i) {
         case 0:
